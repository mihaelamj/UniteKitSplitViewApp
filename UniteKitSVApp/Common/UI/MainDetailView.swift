...
open class MainDetailView: BaseView {
  ...
  private(set) public lazy var textLayer: CATextLayer = {
    let tl = CATextLayer()
    tl.foregroundColor = UKColor.white.cgColor
    ...
    tl.borderColor = UKColor.black.cgColor
    tl.borderWidth = 5.0
    
    // TODO: refactor This -
    #if os(OSX)
    tl.isGeometryFlipped = isFlipped
    tl.contentsScale = UKScreen.main?.scale ?? 2.0
    #endif
    
    // TODO: refactor This -
    #if os(iOS) || os(tvOS)
    tl.contentsScale = UKScreen.main.scale
    #endif
    
    return tl
  }()
  
  private(set) public var contentView: BaseView?
...
private extension MainDetailView {
  ...
  func makeContentView(name: String?) -> BaseView? {
